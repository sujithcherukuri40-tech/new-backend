namespace PavamanDroneConfigurator.Core.Models;

/// <summary>
/// Static class containing Terms and Conditions content for Kapil Future Tech Drone Configurator.
/// </summary>
public static class TermsAndConditions
{
    public const string Copyright = "© 2025 Kapil Future Tech";
    
    public const string Title = "Terms and Conditions";
    
    public const string LastUpdated = "January 2025";
    
    public const string FullText = @"TERMS AND CONDITIONS
Last Updated: January 2025

IMPORTANT: PLEASE READ THESE TERMS AND CONDITIONS CAREFULLY BEFORE USING KAPIL FUTURE TECH DRONE CONFIGURATOR.

1. ACCEPTANCE OF TERMS

By downloading, installing, or using Kapil Future Tech Drone Configurator (""the Software""), you acknowledge that you have read, understood, and agree to be bound by these Terms and Conditions. If you do not agree to these terms, do not use the Software.

Your use of the Software constitutes acceptance of these terms and creates a binding legal agreement between you and Kapil Future Tech (""the Company"", ""we"", ""us"", or ""our"").

2. LICENSE GRANT

Subject to your compliance with these Terms and Conditions, Kapil Future Tech grants you a limited, non-exclusive, non-transferable, revocable license to:

• Download, install, and use the Software on devices you own or control
• Use the Software for configuring, calibrating, and managing compatible drone systems
• Access features and services provided through the Software

This license does not include:
• The right to modify, reverse engineer, decompile, or disassemble the Software
• The right to distribute, sublicense, lease, or rent the Software to third parties
• The right to remove or alter any proprietary notices or labels on the Software
• The right to use the Software for commercial purposes without prior written consent

3. USER RESPONSIBILITIES

By using this Software, you agree to:

Safety Compliance:
• Follow all applicable local, national, and international aviation laws and regulations
• Ensure proper training and certification before operating any drone systems
• Conduct pre-flight safety checks before each flight operation
• Operate drones only in approved airspaces and conditions
• Maintain visual line of sight during flight operations where required by law

Proper Use:
• Use the Software only with compatible and authorized drone systems
• Follow all calibration procedures and safety warnings provided by the Software
• Not modify drone parameters beyond safe operating limits
• Verify all configuration changes before flight
• Maintain proper documentation of all configuration changes

Account Security:
• Maintain the confidentiality of your account credentials
• Notify us immediately of any unauthorized access to your account
• Accept responsibility for all activities under your account

4. PRIVACY AND DATA COLLECTION

Information We Collect:
• Account information (name, email, organization)
• Device and connection information for drone configuration
• Usage data to improve the Software
• Parameter configurations and calibration data

How We Use Your Information:
• To provide and improve the Software's functionality
• To communicate important updates and safety notices
• To provide technical support
• To comply with legal obligations

Data Protection:
• We implement industry-standard security measures to protect your data
• We do not sell your personal information to third parties
• You may request deletion of your account and associated data

For detailed information, please refer to our Privacy Policy.

5. DISCLAIMERS

AS-IS BASIS:
THE SOFTWARE IS PROVIDED ""AS IS"" AND ""AS AVAILABLE"" WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.

NO WARRANTY OF OPERATION:
WE DO NOT WARRANT THAT:
• The Software will meet your specific requirements
• The Software will be uninterrupted, timely, secure, or error-free
• The results obtained from using the Software will be accurate or reliable
• Any errors in the Software will be corrected

DRONE OPERATION RISKS:
You acknowledge that:
• Drone operation involves inherent risks including property damage, personal injury, and death
• Improper configuration or calibration can result in loss of control, crashes, or other incidents
• Environmental conditions may affect drone performance regardless of configuration
• The Software is a tool to assist configuration and does not guarantee safe operation

6. LIMITATION OF LIABILITY

TO THE MAXIMUM EXTENT PERMITTED BY LAW:

KAPIL FUTURE TECH SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING BUT NOT LIMITED TO:
• Loss of profits, revenue, or data
• Property damage or personal injury
• Business interruption
• Any damages arising from drone crashes or malfunctions
• Any damages resulting from your reliance on the Software

IN NO EVENT SHALL OUR TOTAL LIABILITY EXCEED THE AMOUNT PAID BY YOU, IF ANY, FOR THE SOFTWARE IN THE TWELVE (12) MONTHS PRECEDING THE CLAIM.

7. INDEMNIFICATION

You agree to indemnify, defend, and hold harmless Kapil Future Tech, its officers, directors, employees, agents, and affiliates from and against any and all claims, damages, losses, costs, and expenses (including reasonable attorneys' fees) arising from:

• Your use or misuse of the Software
• Your violation of these Terms and Conditions
• Your violation of any applicable laws or regulations
• Your drone operations and any resulting incidents
• Any claims by third parties related to your use of the Software

8. UPDATES AND MODIFICATIONS

Software Updates:
• We may release updates to improve functionality, security, or performance
• Some updates may be mandatory for continued use of the Software
• Updates may modify or remove features at our discretion

Terms Modifications:
• We reserve the right to modify these Terms at any time
• Material changes will be communicated through the Software or via email
• Continued use after changes constitutes acceptance of modified terms
• You are responsible for reviewing terms periodically

9. TERMINATION

We may terminate or suspend your access to the Software:
• For violation of these Terms and Conditions
• For any conduct we deem harmful to other users or the Company
• At our sole discretion with or without cause

Upon termination:
• Your license to use the Software is immediately revoked
• You must cease all use and delete all copies of the Software
• Provisions of these Terms that by their nature should survive will survive

10. INTELLECTUAL PROPERTY

All intellectual property rights in the Software, including but not limited to:
• Software code, algorithms, and architecture
• User interface designs and graphics
• Documentation and training materials
• Trademarks, logos, and branding

Are and shall remain the exclusive property of Kapil Future Tech or its licensors.

11. EXPORT COMPLIANCE

You agree to comply with all applicable export and re-export control laws and regulations, including:
• U.S. Export Administration Regulations
• International Traffic in Arms Regulations
• Any applicable local export control laws

12. GOVERNING LAW AND DISPUTE RESOLUTION

These Terms shall be governed by and construed in accordance with the laws of India, without regard to conflict of law principles.

Any disputes arising from these Terms or your use of the Software shall be resolved through:
• Good faith negotiation between the parties
• Binding arbitration in accordance with applicable arbitration rules
• Courts of competent jurisdiction in India, if arbitration is not available

13. SEVERABILITY

If any provision of these Terms is found to be unenforceable or invalid, that provision shall be limited or eliminated to the minimum extent necessary, and the remaining provisions shall remain in full force and effect.

14. ENTIRE AGREEMENT

These Terms, together with our Privacy Policy and any additional terms for specific features, constitute the entire agreement between you and Kapil Future Tech regarding the Software and supersede all prior agreements and understandings.

15. CONTACT INFORMATION

For questions about these Terms and Conditions, please contact:

Kapil Future Tech
Email: support@kapilfuturetech.com
Website: www.kapilfuturetech.com

By using Kapil Future Tech Drone Configurator, you acknowledge that you have read, understood, and agree to be bound by these Terms and Conditions.

© 2025 Kapil Future Tech. All rights reserved.";

    /// <summary>
    /// Short summary for display in compact areas
    /// </summary>
    public const string ShortSummary = @"By using Kapil Future Tech Drone Configurator, you agree to:
• Follow all applicable aviation laws and regulations
• Use the Software responsibly and safely
• Accept that drone operation involves inherent risks
• Comply with our Privacy Policy

Please read the full Terms and Conditions before proceeding.";
}
